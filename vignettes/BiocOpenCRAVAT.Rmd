...
`queryOC(chr=""chr7", pos="140753336", annotators=c("pubmed", "segway_breast", 
    "chasmplus_BRCA"))
```{r lkex}
var_in_tx
```
...
names(vcon)
```
A collapsible tree representation is easy to produce:
```{r dolv}
listviewer::jsonedit(vcon, height=200L)
```
